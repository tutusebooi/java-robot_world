...
/**
 * The forward command is a command that moves the robot forward for a certain distance and checks if the new updated position is allowed.
 */
public class MoveForwardCommand extends Commands {

    public MoveForwardCommand(GsonRequest gsonRequest) {
        super(gsonRequest);
    }
    ...
    public GsonResponse executeCommand(World world) {
        ...
        if (world.updatePosition(numberOfSteps, robot)) {
            if (!oldPosition.equals(robot.getPosition())) {
                robot.setStatus("Moved forward by " + numberOfSteps + " steps.");
            }
        } else {

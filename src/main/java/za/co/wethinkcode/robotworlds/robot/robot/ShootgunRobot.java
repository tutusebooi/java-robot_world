...
public class ShootgunRobot extends AbstractRobot {

    public ShootgunRobot(String name) {
        super(name);
    }

    public ShootgunRobot(String name, String typeOfRobot, int bulletShootCapacity, int distanceOfShoot) {
        super(name, typeOfRobot, bulletShootCapacity, distanceOfShoot);
    }

    @Override
    public String theTypeOfRobotSet() {
        return "Shootgun Shooter Robot";
    }
    ...
    public boolean executeShootCommand(World world) {
        ...
        int distanceOfShot = getRobotBullet().getDistanceOfShoot();

        if (Direction.NORTH.equals(this.getCurrentDirection())) {
            int x = this.getPosition().getX();

            for (int y = this.getPosition().getY()+1; y <= this.getPosition().getY()+distanceOfShot; y++) {
            ...
            for (int x = this.getPosition().getX()+1; x <= this.getPosition().getX()+distanceOfShot; x++) {
                this.getRobotBullet().setPositionOfBullet(new Position(x, y));
                for (AbstractRobot robotEnemy: world.getRobots()) {
                    if (!this.getName().equals(robotEnemy.getName()) && this.getRobotBullet().didBulletHitRobot(robotEnemy)) {
                        robotEnemy.getRobotShield().removeShieldAfterHit();
